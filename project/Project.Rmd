...
classify_text <- function(text, word_pair) {
  ...
  # Find the frequent words in the text
  freq_words <- words[words %in% word_pair$word]
  
  if (length(freq_words) == 0) {
    return(NA)  # Return NA if no frequent words are found
  }
...
test <- duke_text$text[1]
duke_texts <- duke_text$text
...
# Apply the classify_text function to each text in duke_text$text
duke_text <- duke_text %>%
