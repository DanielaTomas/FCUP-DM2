...
```{r}
duke_texts <- subset(data_posts_content, id == "duke.csv")
duke_texts <- duke_texts$text

duke_vc <- VectorSource(duke_texts)
duke_corpus <- Corpus(duke_vc)

duke_texts <- cleanup(duke_corpus, c("new", "will", "change", "north", "can", "first", "year", "carolina", "years", "summer", "meet", "one"))
duke_dtm <- DocumentTermMatrix(duke_texts)

duke_dtm.tfidf <- weightTfIdf(duke_dtm)

duke_mdf <- as_tibble(as.matrix(duke_dtm.tfidf))

duke_mdf.freq <- duke_mdf %>%
  select(findFreqTerms(duke_dtm, 30)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

duke_mdf.freq$key <- 
  factor(duke_mdf.freq$key,
         levels = duke_mdf.freq$key[order(duke_mdf.freq$value)])

ggplot(duke_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

duke_word_dictionary <- as.tibble(duke_mdf.freq$key)
...
duke_word_pair <- as.tibble(pairing(duke_word_dictionary, duke_word_category))
...
```{r}
epfl_texts <- subset(data_posts_content, id == "epfl.csv")
epfl_texts <- epfl_texts$text

epfl_vc <- VectorSource(epfl_texts)
epfl_corpus <- Corpus(epfl_vc)

epfl_texts <- cleanup(epfl_corpus, c("new", "amp", "can", "will", "one", "now", "–", "via", "portrait"))
epfl_dtm <- DocumentTermMatrix(epfl_texts)

epfl_dtm.tfidf <- weightTfIdf(epfl_dtm)

epfl_mdf <- as_tibble(as.matrix(epfl_dtm.tfidf))

epfl_mdf.freq <- epfl_mdf %>%
  select(findFreqTerms(epfl_dtm, 30)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

epfl_mdf.freq$key <- 
  factor(epfl_mdf.freq$key,
         levels = epfl_mdf.freq$key[order(epfl_mdf.freq$value)])

ggplot(epfl_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

epfl_word_dictionary <- as.tibble(epfl_mdf.freq$key)
...
epfl_word_pair <- as.tibble(pairing(epfl_word_dictionary, epfl_word_category))
...
```{r}
goe_texts <- subset(data_posts_content, id == "goe.csv")
goe_texts <- goe_texts$text

goe_vc <- VectorSource(goe_texts)
goe_corpus <- Corpus(goe_vc)

goe_texts <- cleanup(goe_corpus, c("can", "new", "amp", "will"))
goe_dtm <- DocumentTermMatrix(goe_texts)

goe_dtm.tfidf <- weightTfIdf(goe_dtm)

goe_mdf <- as_tibble(as.matrix(goe_dtm.tfidf))

goe_mdf.freq <- goe_mdf %>%
  select(findFreqTerms(goe_dtm, 15)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

goe_mdf.freq$key <- 
  factor(goe_mdf.freq$key,
         levels = goe_mdf.freq$key[order(goe_mdf.freq$value)])

ggplot(goe_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

goe_word_dictionary <- as.tibble(goe_mdf.freq$key)

goe_word_category <- c("Research", "Research", "Image", "Education", "Engagement", "Image", "Engagement", "Education")

goe_word_pair <- as.tibble(pairing(goe_word_dictionary, goe_word_category))
...
```{r}
harvard_texts <- subset(data_posts_content, id == "harvard.csv")
harvard_texts <- harvard_texts$text

harvard_vc <- VectorSource(harvard_texts)
harvard_corpus <- Corpus(harvard_vc)

harvard_texts <- cleanup(harvard_corpus, c("new", "can", "will", "summer", "year", "first", "may", "-", "recent", "years", "one", "said", "time", "many", "world", "change"))
harvard_dtm <- DocumentTermMatrix(harvard_texts)

harvard_dtm.tfidf <- weightTfIdf(harvard_dtm)

harvard_mdf <- as_tibble(as.matrix(harvard_dtm.tfidf))

harvard_mdf.freq <- harvard_mdf %>%
  select(findFreqTerms(harvard_dtm, 50)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

harvard_mdf.freq$key <- 
  factor(harvard_mdf.freq$key,
         levels = harvard_mdf.freq$key[order(harvard_mdf.freq$value)])

ggplot(harvard_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

harvard_word_dictionary <- as.tibble(harvard_mdf.freq$key)
...
harvard_word_pair <- as.tibble(pairing(harvard_word_dictionary, harvard_word_category))
...
```{r}
leicester_texts <- subset(data_posts_content, id == "leicester.csv")
leicester_texts <- leicester_texts$text

leicester_vc <- VectorSource(leicester_texts)
leicester_corpus <- Corpus(leicester_vc)

leicester_texts <- cleanup(leicester_corpus, c("👉", "day", "new", "clear", "year", "can", "will", "time", "space", "one", "first"))
leicester_dtm <- DocumentTermMatrix(leicester_texts)

leicester_dtm.tfidf <- weightTfIdf(leicester_dtm)

leicester_mdf <- as_tibble(as.matrix(leicester_dtm.tfidf))

leicester_mdf.freq <- leicester_mdf %>%
  select(findFreqTerms(leicester_dtm, 60)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

leicester_mdf.freq$key <- 
  factor(leicester_mdf.freq$key,
         levels = leicester_mdf.freq$key[order(leicester_mdf.freq$value)])

ggplot(leicester_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

leicester_word_dictionary <- as.tibble(leicester_mdf.freq$key)
...
leicester_word_pair <- as.tibble(pairing(leicester_word_dictionary, leicester_word_category))
...
```{r}
manchester_texts <- subset(data_posts_content, id == "manchester.csv")
manchester_texts <- manchester_texts$text

manchester_vc <- VectorSource(manchester_texts)
manchester_corpus <- Corpus(manchester_vc)

manchester_texts <- cleanup(manchester_corpus, c("👇", "can", "just", "will", "get", "well", "one", "help", "now", "new", "read", "congratulations"))
manchester_dtm <- DocumentTermMatrix(manchester_texts)

manchester_dtm.tfidf <- weightTfIdf(manchester_dtm)

manchester_mdf <- as_tibble(as.matrix(manchester_dtm.tfidf))

manchester_mdf.freq <- manchester_mdf %>%
  select(findFreqTerms(manchester_dtm, 50)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

manchester_mdf.freq$key <- 
  factor(manchester_mdf.freq$key,
         levels = manchester_mdf.freq$key[order(manchester_mdf.freq$value)])

ggplot(manchester_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

manchester_word_dictionary <- as.tibble(manchester_mdf.freq$key)
...
manchester_word_pair <- as.tibble(pairing(manchester_word_dictionary, manchester_word_category))
...
```{r}
mit_texts <- subset(data_posts_content, id == "mit.csv")
mit_texts <- mit_texts$text

mit_vc <- VectorSource(mit_texts)
mit_corpus <- Corpus(mit_vc)

mit_texts <- cleanup(mit_corpus, c("new", "can", "says", "“", "’", "—", "", "may", "first", "will", "using", "way", "one", "science"))
mit_dtm <- DocumentTermMatrix(mit_texts)

mit_dtm.tfidf <- weightTfIdf(mit_dtm)

mit_mdf <- as_tibble(as.matrix(mit_dtm.tfidf))

mit_mdf.freq <- mit_mdf %>%
  select(findFreqTerms(mit_dtm, 40)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

mit_mdf.freq$key <- 
  factor(mit_mdf.freq$key,
         levels = mit_mdf.freq$key[order(mit_mdf.freq$value)])

ggplot(mit_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

mit_word_dictionary <- as.tibble(mit_mdf.freq$key)
...
mit_word_pair <- as.tibble(pairing(mit_word_dictionary, mit_word_category))
...
```{r}
sb_texts <- subset(data_posts_content, id == "sb.csv")
sb_texts <- sb_texts$text

sb_vc <- VectorSource(sb_texts)
sb_corpus <- Corpus(sb_vc)

sb_texts <- cleanup(sb_corpus, c("new", "—", "will", "week", "can", "amp", "via", "now", "future", "first"))
sb_dtm <- DocumentTermMatrix(sb_texts)

sb_dtm.tfidf <- weightTfIdf(sb_dtm)

sb_mdf <- as_tibble(as.matrix(sb_dtm.tfidf))

sb_mdf.freq <- sb_mdf %>%
  select(findFreqTerms(sb_dtm, 30)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

sb_mdf.freq$key <- 
  factor(sb_mdf.freq$key,
         levels = sb_mdf.freq$key[order(sb_mdf.freq$value)])

ggplot(sb_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

sb_word_dictionary <- as.tibble(sb_mdf.freq$key)
...
sb_word_pair <- as.tibble(pairing(sb_word_dictionary, sb_word_category))
...
```{r}
stanford_texts <- subset(data_posts_content, id == "stanford.csv")
stanford_texts <- stanford_texts$text

stanford_vc <- VectorSource(stanford_texts)
stanford_corpus <- Corpus(stanford_vc)

stanford_texts <- cleanup(stanford_corpus, c("new", "will"))
stanford_dtm <- DocumentTermMatrix(stanford_texts)

stanford_dtm.tfidf <- weightTfIdf(stanford_dtm)

stanford_mdf <- as_tibble(as.matrix(stanford_dtm.tfidf))

stanford_mdf.freq <- stanford_mdf %>%
  select(findFreqTerms(stanford_dtm, 25)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

stanford_mdf.freq$key <- 
  factor(stanford_mdf.freq$key,
         levels = stanford_mdf.freq$key[order(stanford_mdf.freq$value)])

ggplot(stanford_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

stanford_word_dictionary <- as.tibble(stanford_mdf.freq$key)

stanford_word_category <- c("Image", "Image", "Research", "Education", NA)

stanford_word_pair <- as.tibble(pairing(stanford_word_dictionary, stanford_word_category))
...
```{r}
trinity_texts <- subset(data_posts_content, id == "trinity.csv")
trinity_texts <- trinity_texts$text

trinity_vc <- VectorSource(trinity_texts)
trinity_corpus <- Corpus(trinity_vc)

trinity_texts <- cleanup(trinity_corpus, c("amp", "read", "new", "can", "will", "week", "work", "great", "day", "visit", "irish", "first", "led", "congratulations"))
trinity_dtm <- DocumentTermMatrix(trinity_texts)

trinity_dtm.tfidf <- weightTfIdf(trinity_dtm)

trinity_mdf <- as_tibble(as.matrix(trinity_dtm.tfidf))

trinity_mdf.freq <- trinity_mdf %>%
  select(findFreqTerms(trinity_dtm, 40)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

trinity_mdf.freq$key <- 
  factor(trinity_mdf.freq$key,
         levels = trinity_mdf.freq$key[order(trinity_mdf.freq$value)])

ggplot(trinity_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

trinity_word_dictionary <- as.tibble(trinity_mdf.freq$key)
...
trinity_word_pair <- as.tibble(pairing(trinity_word_dictionary, trinity_word_category))
...
```{r}
wv_texts <- subset(data_posts_content, id == "wv.csv")
wv_texts <- wv_texts$text

wv_vc <- VectorSource(wv_texts)
wv_corpus <- Corpus(wv_vc)

wv_texts <- cleanup(wv_corpus, c("💛💙", "🙌", "👉", "happy", "day", "see", "week", "great", "can", "will", "well", "know", "new", "now", "get", "just"))
wv_dtm <- DocumentTermMatrix(wv_texts)

wv_dtm.tfidf <- weightTfIdf(wv_dtm)

wv_mdf <- as_tibble(as.matrix(wv_dtm.tfidf))

wv_mdf.freq <- wv_mdf %>%
  select(findFreqTerms(wv_dtm, 40)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

wv_mdf.freq$key <- 
  factor(wv_mdf.freq$key,
         levels = wv_mdf.freq$key[order(wv_mdf.freq$value)])

ggplot(wv_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

wv_word_dictionary <- as.tibble(wv_mdf.freq$key)

#wv_word_category <- c(NA, "Image", "Image", NA, "Image", "Image", "Engagement", "Education", "Education", "Education", NA)

#wv_word_pair <- as.tibble(pairing(wv_word_dictionary, wv_word_category))

print(wv_word_dictionary)
```

# yale

```{r}
yale_texts <- subset(data_posts_content, id == "yale.csv")
yale_texts <- yale_texts$text

yale_vc <- VectorSource(yale_texts)
yale_corpus <- Corpus(yale_vc)

yale_texts <- cleanup(yale_corpus, c("new", "—", "will", "can", "'", "first", "work", "read", "help", "year"))
yale_dtm <- DocumentTermMatrix(yale_texts)

yale_dtm.tfidf <- weightTfIdf(yale_dtm)

yale_mdf <- as_tibble(as.matrix(yale_dtm.tfidf))

yale_mdf.freq <- yale_mdf %>%
  select(findFreqTerms(yale_dtm, 70)) %>%
  summarise_all(sum) %>%
  gather() %>%
  arrange(desc(value))

yale_mdf.freq$key <- 
  factor(yale_mdf.freq$key,
         levels = yale_mdf.freq$key[order(yale_mdf.freq$value)])

ggplot(yale_mdf.freq, aes(x=key, y=value)) +
  geom_bar(stat="identity") +
  labs(x="terms", y="freq") + coord_flip()

yale_word_dictionary <- as.tibble(yale_mdf.freq$key)
...
yale_word_pair <- as.tibble(pairing(yale_word_dictionary, yale_word_category))
