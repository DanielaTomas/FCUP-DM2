...
cluster_table <- merge(cluster_table, select(total_stats, id, total_views, total_likes, total_replys), by = "id", all=TRUE)

# Joining attribute like_ratio (ratio of total number of likes) and replys_ratio (ratio of total number of replies) per HEI
cluster_table <- merge(cluster_table, select(hei_ratios, id, like_ratio, replys_ratio), by = "id", all=TRUE)
...
cluster_data <- select(cluster_table, -id)  # Exclude 'id' column

# Scale the data (optional but recommended for K-means)
scaled_data <- scale(cluster_data)

# Determine the number of clusters (you may need to adjust this)
num_clusters <- 3
...
cluster_assignments <- data.frame(id = cluster_table$id, cluster = kmeans_result$cluster)
