...
```{r}
classify_text <- function(text, rules, word_pair) {
  # Tokenize the text into words
  words <- unlist(strsplit(text, "\\W+"))
  
  # Find the category with the highest confidence rule
  categories <- 
  category_scores <- sapply(categories, function(category) {
    category_rules <- subset(rules, rhs %in% category)
    max(sapply(category_rules, function(rule) {
      antecedent <- labels(lhs(rule))
      consequent <- labels(rhs(rule))
      sum(antecedent %in% words) / length(antecedent) * confidence(rule)
    }), na.rm = TRUE)
  })
  
  # Return the category with the highest score
  categories[which.max(category_scores)]
}
```
...
duke_word_dictionary <- as_tibble(duke_mdf.freq$key)
...
epfl_word_dictionary <- as_tibble(epfl_mdf.freq$key)
...
goe_word_dictionary <- as_tibble(goe_mdf.freq$key)
...
harvard_word_dictionary <- as_tibble(harvard_mdf.freq$key)
...
leicester_word_dictionary <- as_tibble(leicester_mdf.freq$key)
...
manchester_word_dictionary <- as_tibble(manchester_mdf.freq$key)
...
mit_word_dictionary <- as_tibble(mit_mdf.freq$key)
...
sb_word_dictionary <- as_tibble(sb_mdf.freq$key)
...
stanford_word_dictionary <- as_tibble(stanford_mdf.freq$key)
...
trinity_word_dictionary <- as_tibble(trinity_mdf.freq$key)
...
wv_word_dictionary <- as_tibble(wv_mdf.freq$key)
...
yale_word_dictionary <- as_tibble(yale_mdf.freq$key)
